"""Reading a figure off a document, and checking whether the document adds up.

The investigation looks at photographs: an invoice, a receipt, a screenshot of an
order confirmation. Two of the things it reads there are money, and both are easy to
get quietly wrong.

**Reading a figure.** A price on a photograph is a piece of text, and the text says
more than the number does. One of ShipBob's sample claims shows an order screen
reading `£55.95` while ShipBob's own record for the same product says `52.00`. Read as
a bare number those are two prices that disagree by three dollars. Read as written,
one of them is not in dollars at all, and a claim that looks comfortably under the
hundred-dollar cap may be over it. So a figure is read here **with whatever named its
currency still attached** — the symbol as it was written, or the three-letter code —
and the number itself is kept as an exact decimal. Nothing in this file converts one
currency into another, and nothing here decides that a figure with no symbol is
dollars. Both of those are judgements, and they belong to whoever holds the rates.

**Checking the document.** A total printed on a photograph is a number somebody's eyes
or a model's guess produced. It is also a number the document itself can be wrong
about: one sample sales order lists three items adding up to `46.93`, prints a subtotal
of `49.85`, prints its tax total as `0.00`, and prints a final total of `49.42`. So the
totals are **recomputed from the lines** rather than believed, and every place the
document disagrees with itself is reported. The arithmetic happens here, in exact
decimals, and never inside a model's answer — a model asked to add three prices will
usually get it right, and "usually" is not arithmetic.

Everything that is not certain comes back as "I could not read this" rather than as a
guess. A figure nobody could read is a figure a person should look at (NFR-4), and a
guessed one is a payout nobody can check.

**No requirement covers this; see DESIGN.md.** REQUIREMENTS.md never mentions currency,
never says what to do when a document contradicts itself, and the API carries no
currency field anywhere. The nearest requirements are FR-1.21 and FR-1.20, which say who
decides an amount and what caps it; FR-1.13, which forbids narrowing two candidate
readings to one; NFR-1, which asks for the same answer twice; and FR-0.7 with NFR-7,
which put judgement values in the claim policy rather than in a branch.

Nothing here reaches out to anything and nothing here reads a clock. The same text
