"""How sure we can be that a claimed product is one of the products on the order.

`claim_line.py` already ties a claimed product to an order line, but it can only ever say
"exactly one line matches", "no line matches", or "more than one line matches" — an
all-or-nothing rule built for pricing, where a wrong guess would misprice a payout. That
rule is too brittle for a rep trying to work out *what a merchant is describing*, because
the real data very rarely spells a product's name the same way twice:

- ShipBob calls it `Blue Razz Liquid Carnitine`; the customer's own invoice says
  `liquid carnitine 3000`.
- ShipBob calls it `2.5LBS White Chocolate Raspberry Huge Whey`; the customer's invoice
  says `huge whey | protein powder`.
- ShipBob calls it `Bomb Popsicle Wrecked Pre-Workout`; the customer's invoice says
  `wrecked pre workout | strongest pre-workout`.
- Product codes drift the same way: ShipBob's `A00299` becomes `A00299-LV-8-N` on a
  receipt, and CASE-1003's merchant names two damaged products in prose — "L Carnitine",
  "Liquid glycerol" — against a six-line order with nothing spelled quite the way the
  order does.

None of those pairs is an exact match of anything, so the exact-text rule would report
every one of them as "not on the order" — the least useful answer there is, since the
product plainly *is* on the order and a rep would spot it in a second.

This module reports every order line that could plausibly be the claimed product, with a
score for how sure the match is and one sentence saying why, instead of forcing a single
yes-or-no answer. **It never narrows two equally good candidates down to one** — that is
exactly the judgement FR-1.13 reserves for a person, because two similar products can cost
different amounts, and picking one quietly would invent the payout.

`(no requirement covers this; see DESIGN.md)`. It was written after reading the sample
data rather than from anything REQUIREMENTS.md asks for; the nearest ideas already in the
requirements are FR-1.13 (never narrow two candidates to one) and FR-1a.2 (a claimed
product has to be tied to a real one on the order).

Nothing here reaches out to anything, reads a clock, or iterates a plain `set` for
anything that affects the order results come back in — the same claimed product against
the same order lines always produces the same candidates, in the same order (NFR-1).
"""
