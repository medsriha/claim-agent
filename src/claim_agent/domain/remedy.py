"""What a merchant's own words are actually asking for, read by keyword rather than by a model.

A damaged-in-transit claim defaults to a reimbursement recommendation, but that is not
always what the merchant asked for. Two real cases show why this matters:

- CASE-1004's merchant wrote: *"How can I get a lid replacement for my roller? It broke
  pretty quickly after receiving - thought I'd ask!"* That is a request for one broken
  part, not the whole product, on a claim filed as "Damaged in Transit", 73 days after
  delivery, on a case already closed. A reimbursement recommendation would answer a
  question nobody asked.
- CASE-1002's merchant wrote: *"please either refund me in its entirety or send me my
  package in its entirety it's your choice"* — two remedies, offered as alternatives, in
  the merchant's own words.

This module reads the text an investigation has already pulled out of an email or a
screenshot and reports, by a small set of fixed keyword rules, which of a handful of
remedies it recognisably asks for. `unclear` is the default and a perfectly good answer:
when nothing recognisable is there, this says so rather than guessing at what the
merchant meant.

**This is deliberately shallow, and that is worth saying plainly rather than dressing it
up.** Keyword rules over free text will miss politeness ("I don't suppose I could get a
new one?"), sarcasm, and anything phrased indirectly. The model that already read the
text out of an image or an email is better at understanding what somebody is asking for
than a fixed list of phrases ever will be. This module exists to give a **representative**
a structured second opinion they can check the model's own reading against — a plain,
auditable "here is exactly the word that made me think 'refund'" — not to overrule the
model or to replace its judgement of the same text. Whether this belongs in the
investigation's own hands at all, rather than only in what a rep sees afterwards, is
...
case data rather than from anything REQUIREMENTS.md asks for. NFR-4 ("fail toward the
human") is the nearest existing idea: an unclear reading is reported as unclear rather
than resolved by a guess.

Nothing here reaches out to anything, reads a clock, or calls a model. The same text
always produces the same reading (NFR-1).
"""
